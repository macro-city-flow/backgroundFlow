...
class MDN2(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, feature_dim: int, gamma: int, **kwargs):
        ...
        self._gamma = gamma
        self._mu_weights = nn.Parameter(torch.FloatTensor(
            self._gamma, self._output_dim, self._input_dim))
        self._mu_bias = nn.Parameter(torch.FloatTensor(
            self._gamma, self._output_dim, self._feature_dim))
        self._sigma_weights = nn.Parameter(torch.FloatTensor(
            self._gamma, self._output_dim, self._input_dim))
        self._sigma_bias = nn.Parameter(torch.FloatTensor(
            self._gamma, self._output_dim, self._feature_dim))
        self._pi_weights = nn.Parameter(torch.FloatTensor(
            self._gamma, self._output_dim, self._input_dim))
        self._pi_bias = nn.Parameter(torch.FloatTensor(
            self._gamma, self._output_dim, self._feature_dim))
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_normal_(self._pi_weights)
        nn.init.xavier_normal_(self._pi_bias)
        return
    ...
    def forward(self, inputs):
        ...
        weights = self._regularize(self._pi_weights @ inputs + self._pi_bias)
