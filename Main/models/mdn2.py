...
class MDN2(nn.Module):
    def __init__(self,input_dim:int,output_dim:int,feature_dim:int,gamma:int, **kwargs):
        ...
        self._pi_weights = nn.Parameter(torch.FloatTensor(self._gamma,self._output_dim,self._input_dim))
        self._pi_bias = nn.Parameter(torch.FloatTensor(self._gamma,self._output_dim,self._feature_dim))
