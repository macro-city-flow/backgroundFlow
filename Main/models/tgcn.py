...
class TGCN(nn.Module):
    def __init__(self, adj_path, feature_dim: int, input_dim: int, hidden_dim: int, output_dim: int, fr: float, **kwargs):
        ...
        assert(0 <= fr <= 1)
        self._fr = fr
        self._gcn_layer = GCN(self._adj_path, self._feature_dim,
                              self._input_dim, self._hidden_dim, self._hidden_dim)
        self._gru_layer = GRU(self._feature_dim, self._input_dim,
                              self._hidden_dim, self._output_dim, fr=self._fr)
